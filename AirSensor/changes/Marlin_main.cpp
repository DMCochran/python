    ...
    case 105 : // M105
      if(setTargetedHotend(105)){
        ...
        SERIAL_PROTOCOL_F(degTargetHotend(tmp_extruder),1);
          ...
          SERIAL_PROTOCOL_F(degNewSensor(2),1);
          SERIAL_PROTOCOLPGM(" /");
    ...
    {// M109 - Wait for extruder heater to reach target.
      ...
      cancel_heatup = false;
        ...
        while ( target_direction ? (isHeatingHotend(tmp_extruder)) : (isCoolingHotend(tmp_extruder)&&(CooldownNoWait==false)) ) {
          ...
          if( (millis() - codenum) > 250UL )
          { //Print Temp Reading and remaining time every 0.25 second while heating up/cooling down
            SERIAL_PROTOCOLPGM("T:");
            SERIAL_PROTOCOL_F(degHotend(tmp_extruder),1);
               #if defined(TEMP_BED_PIN) && TEMP_BED_PIN > -1
          SERIAL_PROTOCOLPGM(" B:");
        ...
        for (int8_t cur_extruder = 0; cur_extruder < EXTRUDERS; ++cur_extruder) {
          ...
          SERIAL_PROTOCOL_F(degTargetHotend(cur_extruder),1);
            ...
            SERIAL_PROTOCOL((int)tmp_extruder);
              ...
              if(residencyStart > -1)
              {
                 codenum = ((TEMP_RESIDENCY_TIME * 250UL) - (millis() - residencyStart)) / 250UL;
